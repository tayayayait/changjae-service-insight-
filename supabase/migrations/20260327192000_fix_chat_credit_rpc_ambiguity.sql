-- Hotfix: resolve PL/pgSQL identifier ambiguity in chat credit RPCs.
-- Symptom: 42702 ambiguous column references (owner_key/free_used) on
-- chat_credit_status/chat_credit_consume paths.
-- Policy remains unchanged:
-- - free limit = 2
-- - rolling 24-hour window from free_window_started_at
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM public.chat_credit_events AS e
    WHERE e.owner_key = v_owner_key
      AND e.event_type = 'consume'
      AND e.usage_id = v_usage_id
  ) THEN
    RETURN QUERY
    SELECT
      FALSE,
...
BEGIN
  ...
  v_charged_from := COALESCE(v_consume_event->>'charged_from', '');

  IF v_charged_from = 'paid' THEN
    UPDATE public.chat_credit_wallets AS w
    SET paid_remaining = w.paid_remaining + 1,
        updated_at = NOW()
    WHERE w.owner_key = v_owner_key
    RETURNING * INTO v_wallet;
  ELSIF v_charged_from = 'free' THEN
    UPDATE public.chat_credit_wallets AS w
  ...
  ELSE
    ...
    SELECT
      FALSE,
      'consume_event_missing_charge_source',
      v_wallet.free_used,
      v_wallet.paid_remaining,
      v_free_limit + v_wallet.paid_remaining,
      GREATEST(0, v_free_limit - v_wallet.free_used) + v_wallet.paid_remaining,
...
BEGIN
  ...
  IF EXISTS (
    ...
    FROM public.chat_credit_events AS e
    WHERE e.owner_key = v_owner_key
      AND e.event_type = 'grant_pack'
      AND e.order_number = v_order_number
  ) THEN
    RETURN QUERY
    SELECT
      FALSE,
