...
import { Search, History, ArrowRight, Loader2, UserCheck, AlertTriangle, Clock3 } from "lucide-react";
...
export default function ReportLookupPage() {
  ...
  return (
    <AnalysisPageShell
      categoryId="saju"
      title="내 리포트 찾기"
      subtitle="결제 시 입력한 주문자 정보로 리포트를 다시 확인하세요"
      icon={History}
    ...
    >
      <div className="mx-auto max-w-2xl space-y-8 pb-20">
        ...
        <Card className="overflow-hidden rounded-3xl border-border/70 bg-bg-elevated/65 shadow-xl backdrop-blur-md">
          <CardHeader className="border-b border-border/70 bg-accent-lavender/8 p-6">
            <CardTitle className="inline-flex items-center gap-2 text-lg font-bold">
              <UserCheck className="h-5 w-5 text-accent-lavender" />
              본인 확인
            </CardTitle>
          </CardHeader>
          <CardContent className="p-8">
            <form onSubmit={handleLookup} className="space-y-6">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <Input
                    ...
                    required
                  />
                  <p className="ml-1 text-[11px] text-text-secondary">
              ...
              <Button
                type="submit"
                className="h-14 w-full rounded-2xl bg-primary text-lg font-bold text-primary-foreground transition-all hover:bg-primary-hover"
                disabled={loading}
              >
              ...
              {errorMessage ? (
                <div className="analysis-card-warning flex items-center gap-2 text-sm font-medium text-foreground">
        ...
        {hasSearched ? (
          <div className="animate-in slide-in-from-bottom-4 space-y-4 fade-in duration-500">
            <h3 className="flex items-center gap-2 px-1 text-lg font-bold text-foreground">
              조회 결과
              <span className="text-sm font-normal text-text-secondary">({results?.length || 0}건)</span>
            </h3>

            {loading ? (
              <div className="flex flex-col items-center justify-center space-y-4 py-20">
                <Loader2 className="h-10 w-10 animate-spin text-accent-lavender/50" />
                <p className="font-medium text-text-secondary">리포트를 우주에서 찾는 중...</p>
              </div>
                ...
                {results.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      className="group block rounded-3xl border border-border/70 bg-bg-elevated/65 p-5 shadow-sm transition-all hover:border-accent-lavender hover:shadow-lg"
                    >
                      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div className="min-w-0 space-y-4">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="rounded-md bg-bg-subtle px-2 py-0.5 text-[10px] font-bold uppercase tracking-tighter text-text-secondary tabular-nums">
                              {item.orderNumber}
                            </span>
                            <span className="rounded-md bg-state-success-soft px-2 py-0.5 text-xs font-bold text-state-success">결제완료</span>
                            <span className="rounded-md bg-accent-lavender/15 px-2 py-0.5 text-xs font-bold text-accent-lavender">
                          ...
                          <div className="space-y-1">
                            ...
                            <h4 className="truncate text-lg font-black text-foreground transition-colors group-hover:text-accent-lavender md:text-xl">
                              {getServiceLabel(item.report.service_id)}
                            </h4>
                          </div>
                          <dl className="grid grid-cols-1 gap-3 rounded-2xl border border-border/70 bg-bg-subtle/70 p-4 text-sm sm:grid-cols-3">
                            <div>
                              <dt className="text-[11px] font-semibold text-text-secondary">결제일</dt>
                              <dd className="mt-1 font-semibold text-foreground">{retention.paidDateLabel}</dd>
                            </div>
                            <div>
                              <dt className="text-[11px] font-semibold text-text-secondary">보관 만료일</dt>
                              <dd className="mt-1 font-semibold text-foreground">{retention.expiryDateLabel}</dd>
                            </div>
                            <div>
                              <dt className="text-[11px] font-semibold text-text-secondary">결제 금액</dt>
                              <dd className="mt-1 font-semibold text-foreground">₩{item.amount_krw.toLocaleString()}</dd>
                            </div>
                          </dl>
                        </div>
                        <div className="ml-auto flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-bg-subtle transition-all group-hover:bg-accent-lavender/20 group-hover:text-accent-lavender">
              ...
              <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-border/80 bg-bg-elevated/45 px-6 py-20 text-center">
        ...
        <div className="border-t border-border/70 pt-10">
          <div className="flex flex-col items-center gap-4 text-center">
            <p className="text-sm text-text-secondary">도움이 필요하신가요?</p>
            <div className="flex gap-2">
              ...
              <Link to="/help">
                <Button variant="ghost" className="h-10 rounded-full px-6 text-text-secondary">
                  자주 묻는 질문
                </Button>
              </Link>
            </div>
