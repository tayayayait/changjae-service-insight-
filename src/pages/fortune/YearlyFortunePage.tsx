...
import { useYearlyFortuneFlow } from "@/hooks/fortune/useYearlyFortuneFlow";

export default function YearlyFortunePage() {
  const { baseResult, yearly, isLoading, error, setRetryToken } = useYearlyFortuneFlow();
      ...
      <div className="mx-auto w-full max-w-5xl space-y-5 px-6 py-8">
        <header className="space-y-2">
          ...
          <p className="text-caption text-text-secondary">
            개인 사주를 기준으로 올해의 흐름과 실행 포인트를 제공합니다.
          </p>
        </header>
        ...
        {!isLoading && !baseResult ? (
          <EmptyState
            ...
            description="연간 운세를 보려면 먼저 사주 분석을 완료해 주세요."
        ...
        {!isLoading && !error && yearly ? (
          <>
            <Card className="space-y-4 rounded-lg border-border p-5">
              ...
              <p className="text-[14px] leading-[1.8] text-foreground">{yearly.summary}</p>
              <div className="grid gap-3 md:grid-cols-2">
                <div className="rounded-md bg-bg-subtle p-4">
                  <p className="text-[12px] font-semibold text-text-secondary">집중 포인트</p>
                  <ul className="mt-2 space-y-2 text-[14px] leading-relaxed text-foreground">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {yearly.months.map((item) => (
                <Card key={item.month} className="rounded-lg border-border p-4">
                  ...
                  <p className="mt-2 text-[13px] leading-relaxed text-text-secondary">{item.summary}</p>
                </Card>
              ))}
            </div>
          </>
