...
import { Home, PenSquare, Heart, Star, User } from "lucide-react";
...
const tabs = [
  ...
  { label: "보관함", path: "/mypage", icon: User },
];
...
export function BottomTab() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-bottom-tab border-t border-border bg-bg-elevated/95 backdrop-blur-sm md:hidden"
      ...
      <div className="grid h-[72px] grid-cols-5 items-center">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <Icon size={18} />
              <span className={cn("text-[11px] font-semibold", isActive ? "text-foreground" : "text-text-muted")}>
                {tab.label}
              </span>
            </Link>
          );
